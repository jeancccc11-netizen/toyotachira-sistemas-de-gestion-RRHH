...
export default function Vacaciones() {
  ...
  const handleAprobar = async (id) => { if (!confirm('¿Aprobar?')) return; await api.put(`/vacaciones/solicitudes/${id}/aprobar`); load(); };
  const handleRechazar = async (id) => { if (!confirm('¿Rechazar?')) return; await api.put(`/vacaciones/solicitudes/${id}/rechazar`); load(); };

  const handleSubmit = async (e) => {
    e.preventDefault(); setError('');
    try { await api.post('/vacaciones/solicitudes', form); setForm({ empleado_id: '', fecha_salida: '', fecha_regreso: '', motivo: '' }); setShowForm(false); load(); }
  ...
  return (
    <div>
      <PageHeader title="Vacaciones"
        action={<button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-700"><Plus size={16} /> Nueva Solicitud</button>} />
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-4 mb-6">
          ...
          <div className="flex gap-2 mt-3">
            ...
            <button type="submit" className="px-3 py-1.5 text-sm bg-primary-600 text-white rounded-lg">Enviar</button>
          </div>
        </form>
      )}
          ...
          <tbody className="divide-y divide-gray-100">
            {pendientes.map((s) => (
              <tr key={s.id} className="hover:bg-gray-50">
                ...
                <td>{new Date(s.fecha_regreso).toLocaleDateString()}</td>
                <td className="text-center">{s.dias_solicitados}</td>
                <td className="text-right flex gap-2 justify-end">
                  <button onClick={() => handleAprobar(s.id)} className="p-1.5 bg-green-100 text-green-600 rounded-lg hover:bg-green-200"><Check size={16} /></button>
                  <button onClick={() => handleRechazar(s.id)} className="p-1.5 bg-red-100 text-red-600 rounded-lg hover:bg-red-200"><X size={16} /></button>
                </td>
              </tr>
            ))}
          </tbody>
