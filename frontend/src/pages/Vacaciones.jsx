...
import HistorialList from '../components/vacaciones/HistorialList';
import PeriodoResumen from '../components/vacaciones/PeriodoResumen';
...
export default function Vacaciones() {
  ...
  useEffect(() => { load(); }, []);
  const ap = async (id) => { if (!confirm('¿Aprobar?')) return; await api.put(`/vacaciones/solicitudes/${id}/aprobar`); load(); };
  const re = async (id) => { if (!confirm('¿Rechazar?')) return; await api.put(`/vacaciones/solicitudes/${id}/rechazar`); load(); };
  ...
  return (
    <div>
      <PageHeader title="Vacaciones" action={<button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-700"><Plus size={16} /> Nueva Solicitud</button>} />
      ...
      {tab === 'historial' && <div className="space-y-4"><PeriodoResumen /><HistorialList /></div>}
      {tab === 'pendientes' && (<>
